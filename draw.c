...
/*   Updated: 2024/10/27 12:03:50 by aelbouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_map(t_all *all, int color, int type)
{
	...
	while(i < 10)
	{
		...
		while (j < 10)
		{
			if (type == 1 && (all->cub->y == y || all->cub->x == x || j == 10 || i == 10))
...
void	draw_minimap(t_all *all)
{
	...
	while (i - 3 < 3)
		i++;
	while (all->cub->lines[i - 3])
	{
		j = 0;
		while (j - 3 < 3)
			j++;
		while (all->cub->lines[i][j - 3])
		{
			all->cub->x =  (j - 3) * 10;
			all->cub->y =  (i - 3) * 10;
			draw_mp(all, all->cub->lines[i][j]);
...
void	hor_point(t_all *all, float angle)
{
	...
	if(!(angle > 0 && angle < M_PI))
	{
		yend = floor(all->player->oy / q_size) *  q_size - 0.01f;
		ystep = -1 * q_size;
	}
...
void	ver_point(t_all *all, float angle)
{
	...
	{
		xend = floor(all->player->ox / q_size) *  q_size - 0.01f;
		xstep = -1 * q_size;
	}
	if(angle < 0.5 * M_PI || angle > 1.5 * M_PI)
	{
...
void draw_fc(t_all *all)
{
	...
	while (i < all->res_h)
	{
		...
		while (j < all->res_w)
		{
				// mlx_put_pixel(all->cub->img, j, i, ft_color(50, 10 , 10, 255));
			if (i < all->res_h / 2)
				mlx_put_pixel(all->cub->img, j, i, ft_color(255, 255 , 255, 255));
			else
				mlx_put_pixel(all->cub->img, j, i, ft_color(255, 255 , 255, 255));
				// mlx_put_pixel(all->cub->img, j, i, ft_color(50, 50 , 50, 255));
			j++;
		}
		i++;
	}
...
void	ft_catch(void *d)
{
	...
	{
		y = sin(all->player->angle) * pl_speed;
		x = cos(all->player->angle) * pl_speed;
		slide_in(all , y, x);
	}
	...
	{
		y = sin(all->player->angle + (M_PI / 2)) * pl_speed;
		x = cos(all->player->angle + (M_PI / 2)) * pl_speed;
		slide_in(all , y, x);
	}
...
void ft_draw(t_all *all)
{
	...
	// draw_player(all, ft_color(251,65,88, 255));
	mlx_texture_t *img = mlx_load_png("./images/3.png");
	all->cub->texture = mlx_texture_to_image(all->cub->mlx_ptr, img);
	mlx_get_mouse_pos(all->cub->mlx_ptr, &all->mouse->x, &all->mouse->y);
